...
class Animation {
	...
	updateCurves() {
		...
		for (let i = 0; i < config.curvesNum; ++i ) {
			...
			let curveParam = {
				...
				conrtolX1 : _controlX1,
				conrtolY1 : _controlY1,
				conrtolX2 : _controlX2,
				conrtolY2 : _controlY2 * this.size.h,
				endX : this.size.w,
	...
	drawCurve({startX, startY, conrtolX1, conrtolY1, conrtolX2, conrtolY2, endX, endY, alpha, hue }) {
		this.ctx.strokeStyle = `hsla(${hue}, 100%, 50%, ${alpha})`;
		this.ctx.beginPath();
		this.ctx.moveTo(startX, startY);
		this.ctx.bezierCurveTo(conrtolX1, conrtolY1, conrtolX2, conrtolY2, endX, endY);
		this.ctx.stroke();
	}
	updateCanvas() {
